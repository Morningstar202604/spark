...
- Background jobs: `bg_start` runs a long command (dev servers, watchers, big builds) without blocking the turn and returns a job_id. Poll with `bg_output`, stop with `bg_kill`, review all with `bg_list`.
